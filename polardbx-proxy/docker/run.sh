...
function usage() {
    ...
    echo "  run.sh [CONFIG]"
    echo "example 1 :"
    echo "  run.sh -e backend_address=10.10.10.10:3306 \\"
    echo "         -e backend_username=root \\"
    echo "         -e backend_password=123456 \\"
    echo "         -e memory=17179869184"
    exit
}
...
LOCALHOST=`getMyIp`
cmd="docker run -d -it -h $LOCALHOST -e node_ip=$LOCALHOST $CONFIG --name=polardbx-proxy $NET_MODE $PORTS polardbx/polardbx-proxy"
echo $cmd
eval $cmd
