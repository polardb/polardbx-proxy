...
import com.alibaba.polardbx.proxy.parser.ast.stmt.dml.DMLSelectUnionStatement.UnionOption;
...
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_FOR;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_GROUP;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_JOIN;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_ORDER;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_OUTER;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_PRIMARY;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_UNION;
...
public abstract class MySQLDMLParser extends MySQLParser {
     ...
     * <code>(id (',' id)*)?</code>
     *
     * @return never null or empty. {@link ArrayList} is possible
     */
    ...
    /**
     * <code>( indexName (',' indexName)*)? ')'</code>
     *
     * @return empty list if emtpy id list
     */
    protected List<String> indexNameList() throws SQLSyntaxErrorException {
        if (lexer.token() != IDENTIFIER && lexer.token() != KW_PRIMARY) {
            match(PUNC_RIGHT_PAREN);
            return Collections.emptyList();
        }
        List<String> list;
    ...
    private TableReferences buildTableReferences(TableReference ref) throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == PUNC_COMMA) {
            list = new ArrayList<TableReference>();
            list.add(ref);
            for (; lexer.token() == PUNC_COMMA; ) {
                lexer.nextToken();
                ref = tableReference();
    ...
    private IndexHint hint() throws SQLSyntaxErrorException {
        ...
        match(PUNC_LEFT_PAREN);
        List<String> indexList = indexNameList();
    ...
    protected DMLQueryStatement buildUnionSelect(DMLSelectStatement select) throws SQLSyntaxErrorException {
        ...
        UnionOption option = UnionOption.DEFAULT;
        for (; lexer.token() == KW_UNION; ) {
            lexer.nextToken();
            switch (lexer.token()) {
            case KW_ALL:
                option = UnionOption.ALL;
                lexer.nextToken();
                break;
            case KW_DISTINCT:
                option = UnionOption.DISTINCT;
                lexer.nextToken();
                break;
            }
            select = selectPrimary(true);
    ...
    /**
     * 对于union，如果order by limit在括号里面，则属于子查询的，否则则属于union的
     *
     * @param isUnion 是不是union的一部分
     */
    protected DMLSelectStatement selectPrimary(boolean isUnion) throws SQLSyntaxErrorException {
        ...
        case PUNC_LEFT_PAREN:
            lexer.nextToken();
            DMLSelectStatement select = selectPrimary(false);
            match(PUNC_RIGHT_PAREN);
            return select;
        default:
    ...
    public DMLSelectStatement select(boolean ignoreOrderByAndLimit) throws SQLSyntaxErrorException {
        return new MySQLDMLSelectParser(lexer, exprParser).select(ignoreOrderByAndLimit);
    }

    public DMLQueryStatement selectUnion() throws SQLSyntaxErrorException {
        return new MySQLDMLSelectParser(lexer, exprParser).selectUnion();
    }

    public boolean filterConditionExists(TableReferences tableReferences) {
        if (null == tableReferences) {
            return false;
        }

        boolean result = false;

        for (TableReference tableRef : tableReferences.getTableReferenceList()) {

            if (tableRef.getPrecedence() == TableReference.PRECEDENCE_JOIN) {
                Expression onCond = null;
                if (tableRef instanceof InnerJoin) {
                    final InnerJoin innerJoin = (InnerJoin) tableRef;
                    onCond = innerJoin.getOnCond();
                } else if (tableRef instanceof StraightJoin) {
                    StraightJoin straightJoin = (StraightJoin) tableRef;
                    onCond = straightJoin.getOnCond();
                }
                result = filterExpressionExists(onCond);
            }
    ...
    public boolean filterExpressionExists(Expression expression) {
        if (null == expression) {
            return false;
        }

        if (expression instanceof ComparisionEqualsExpression) {
            ComparisionEqualsExpression equalsExpression = (ComparisionEqualsExpression) expression;
            boolean columnEquals = (equalsExpression.getLeftOprand() instanceof Identifier)
                && (equalsExpression.getRightOprand() instanceof Identifier);
            return !columnEquals;
        } else if (expression instanceof LogicalAndExpression) {
            LogicalAndExpression andExpression = (LogicalAndExpression) expression;

            for (int i = 0; i < andExpression.getArity(); i++) {
                boolean result = filterExpressionExists(andExpression.getOperand(i));
                if (result) {
                    return true;
                }
            }
