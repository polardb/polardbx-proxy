...
public class StringInspector {
     ...
     * is not touched.
     */
    public StringInspector(String source, String openingMarkers, String closingMarkers, String overridingMarkers,
     ...
     * is not touched.
     */
    public StringInspector(String source, int startingPosition, String openingMarkers, String closingMarkers,
        ...
        if (this.defaultSearchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)) {
            if (this.openingMarkers == null || this.closingMarkers == null
                || this.openingMarkers.length() != this.closingMarkers.length()) {
                throw new IllegalArgumentException(MessageFormat.format(
                    "Illegal argument value {0} for openingMarkers and/or {1} for closingMarkers. These cannot be null and must have the same length.",
                    this.openingMarkers, this.closingMarkers));
            }
            if (this.overridingMarkers == null) {
                throw new IllegalArgumentException(MessageFormat.format(
                    "Illegal argument value {0} for overridingMarkers. These cannot be null and must be a sub-set of openingMarkers {1}.",
                    this.overridingMarkers, this.openingMarkers));
            }
    ...
    public int setStopPosition(int pos) {
        ...
        if (pos > this.srcLen) {
            throw new IllegalArgumentException(
                "The stop position must must not be higher than the length of the source string.");
        }
    ...
    private int indexOfNextChar(Set<SearchMode> searchMode) {
        ...
        for (; this.pos < this.stopAt; this.pos++) {
            ...
            if (checkSkipConditions && searchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)
            ...
            } else if (checkSkipConditions && searchMode.contains(SearchMode.SKIP_BLOCK_COMMENTS) && c0 == '/'
                && c1 == '*' && c2 != '!' && c2 != '+') {
                ...
                && (c0 == '-' && c1 == '-' && (Character.isWhitespace(c2) || (dashDashCommentImmediateEnd = c2 == ';')
            ...
            } else if (checkSkipConditions && searchMode.contains(SearchMode.SKIP_HINT_BLOCKS) && c0 == '/' && c1 == '*'
                && c2 == '+') {
            ...
            } else if (checkSkipConditions && searchMode.contains(SearchMode.SKIP_MYSQL_MARKERS) && c0 == '/'
                && c1 == '*' && c2 == '!') {
                ...
                if (c2 == '!') {
                    ...
                    for (; i < NON_COMMENTS_MYSQL_VERSION_REF_LENGTH; i++) {
                        if (this.pos + 1 + i >= this.srcLen || !Character.isDigit(
                            this.source.charAt(this.pos + 1 + i))) {
                            break;
                        }
                    }
                    if (i == NON_COMMENTS_MYSQL_VERSION_REF_LENGTH) {
                        ...
                        if (this.pos >= this.stopAt) {
                            this.pos =
                                this.stopAt - 1; // Reached stop position. Correct position will be set by outer loop.
                        }
                    }
                }
            ...
            } else if (this.inMysqlBlock && checkSkipConditions && searchMode.contains(SearchMode.SKIP_MYSQL_MARKERS)
                && c0 == '*' && c1 == '/') {
    ...
    private int indexOfClosingMarker(Set<SearchMode> searchMode) {
        ...
        while (++this.pos < this.stopAt && ((c0 = this.source.charAt(this.pos)) != closingMarker
            ...
            if (!outerIsAnOverridingMarker && this.overridingMarkers.indexOf(c0) != -1) {
                ...
                int overridingMarkerIndex =
                    this.openingMarkers.indexOf(c0); // OverridingMarkers must be a sub-list of openingMarkers.
                ...
                while (++this.pos < this.stopAt && ((c0 = this.source.charAt(this.pos)) != overridingClosingMarker
    ...
    public int indexOfIgnoreCase(String searchFor, Set<SearchMode> searchMode) {
        ...
        if (Character.isWhitespace(firstCharOfSearchForLc) && this.defaultSearchMode.contains(
    ...
    public int indexOfIgnoreCase(String... searchFor) {
        ...
        if (Character.isWhitespace(searchFor[0].charAt(0)) && this.defaultSearchMode.contains(
            SearchMode.SKIP_WHITE_SPACE)) {
    ...
    public String stripCommentsAndHints() {
        ...
        Set<SearchMode> searchMode =
            EnumSet.of(SearchMode.SKIP_BLOCK_COMMENTS, SearchMode.SKIP_LINE_COMMENTS, SearchMode.SKIP_HINT_BLOCKS);
        if (this.defaultSearchMode.contains(SearchMode.ALLOW_BACKSLASH_ESCAPE)) {
        ...
        while (this.pos < this.stopAt) {
            ...
            if (!this.escaped && this.openingMarkers.indexOf(getChar()) != -1) {
                ...
                if (this.pos - prevPos
                    > 1) { // Non consecutive characters, there was a comment in between. Add a space if needed.
                    if (prevPos > 0 && !Character.isWhitespace(this.source.charAt(prevPos - 1))
