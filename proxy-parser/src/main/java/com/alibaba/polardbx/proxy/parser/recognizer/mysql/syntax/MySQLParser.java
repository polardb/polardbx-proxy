...
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.KW_LIMIT;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.PUNC_DOT;
import static com.alibaba.polardbx.proxy.parser.recognizer.mysql.MySQLToken.SYS_VAR;
...
public abstract class MySQLParser {
    ...
    public Identifier identifier() throws SQLSyntaxErrorException {
        ...
        case QUESTION_MARK:
            ...
            break;
        case LITERAL_CHARS:
            String tempstr = lexer.stringValue().substring(1, lexer.stringValue().length() - 1);
        ...
        for (; lexer.token() == PUNC_DOT; ) {
            ...
            default:
                ...
                if (id != null) {
                    if (lexer.token().name().startsWith("KW_")) {
                        id = new Identifier(id, lexer.stringValue(), lexer.stringValueUppercase());
                        id.setCacheEvalRst(cacheEvalRst);
                        lexer.nextToken();
                        break;
            ...
            case IDENTIFIER:
                if ("OFFSET".equals(lexer.stringValueUppercase())) {
                    ...
                    case LITERAL_NUM_PURE_DIGIT:
                        Number num2 = lexer.integerValue();
                        lexer.nextToken();
                        limitObj = new Limit(num2, num1);
                        limitObj.setPostpositiveOffset(true);
                        return limitObj;
                    case QUESTION_MARK:
                        paramIndex1 = lexer.paramIndex();
                        lexer.nextToken();
                        limitObj = new Limit(createParam(paramIndex1), num1);
                        limitObj.setPostpositiveOffset(true);
                        return limitObj;
                    default:
                        throw err("expect digit or ? after , for limit");
            ...
            case IDENTIFIER:
                if ("OFFSET".equals(lexer.stringValueUppercase())) {
                    ...
                    case LITERAL_NUM_PURE_DIGIT:
                        num1 = lexer.integerValue();
                        lexer.nextToken();
                        limitObj = new Limit(num1, createParam(paramIndex1));
                        limitObj.setPostpositiveOffset(true);
                        ;
                        return limitObj;
                    case QUESTION_MARK:
                        paramIndex2 = lexer.paramIndex();
                        lexer.nextToken();
                        limitObj = new Limit(createParam(paramIndex2), createParam(paramIndex1));
                        limitObj.setPostpositiveOffset(true);
                        return limitObj;
                    default:
                        throw err("expect digit or ? after , for limit");
     ...
     * if none is matched.
     */
    protected int equalsIdentifier(String... expectTextUppercase) throws SQLSyntaxErrorException {
        if (lexer.token() == MySQLToken.IDENTIFIER) {
            String id = lexer.stringValueUppercase();
            for (int i = 0; i < expectTextUppercase.length; ++i) {
                if (expectTextUppercase[i].equals(id)) {
                    return i;
                }
            }
        }
