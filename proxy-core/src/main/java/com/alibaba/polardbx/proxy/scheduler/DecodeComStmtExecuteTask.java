...
public class DecodeComStmtExecuteTask implements ScheduleTask {
    @Override
    public Boolean forward(Scheduler scheduler) throws Exception {
        if (null == scheduler.getRequest() && scheduler.getDecoder() != null) {
            ...
            final ComStmtExecute stmtExecute = new ComStmtExecute();
            stmtExecute.setPreparedStatementContextGetter(id -> {
                assert id == stmtId;
                return preparedStatementContext;
            });
            // skip some param if set by send long data
            stmtExecute.setPreparedStatementParameterSkipper(
                (id, paramIdx) -> {
                    assert id == stmtId;
                    return preparedStatementContext.getLongDataParams()[paramIdx] != null;
                });
            stmtExecute.decode(scheduler.getDecoder(), scheduler.getContext().getCapabilities());
            scheduler.setRequest(stmtExecute);
        }
        return null;
    }
}
