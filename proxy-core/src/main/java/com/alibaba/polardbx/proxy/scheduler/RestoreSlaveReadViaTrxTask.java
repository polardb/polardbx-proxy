...
public class RestoreSlaveReadViaTrxTask implements ScheduleTask {
    @Override
    public Boolean forward(Scheduler scheduler) throws Exception {
        if (null == scheduler.getSlaveRead()) {
            ...
            // go slave state from trx
            if (null == trx) {
                throw new IllegalStateException("Transaction context is null");
            }
            scheduler.setSlaveRead(!trx.isRwTrx());
        }
        return null;
    }
}
