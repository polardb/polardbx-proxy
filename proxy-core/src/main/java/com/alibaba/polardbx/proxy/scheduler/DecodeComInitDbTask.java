...
public class DecodeComInitDbTask implements ScheduleTask {
    @Override
    public Boolean forward(Scheduler scheduler) throws Exception {
        if (null == scheduler.getRequest() && scheduler.getDecoder() != null) {
            final ComInitDb initDb = new ComInitDb();
            initDb.decode(scheduler.getDecoder(), scheduler.getContext().getCapabilities());
            scheduler.setRequest(initDb);
        }
        return null;
    }
}
