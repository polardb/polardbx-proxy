...
public class DecodeComStmtFetchTask implements ScheduleTask {
    @Override
    public Boolean forward(Scheduler scheduler) throws Exception {
        if (null == scheduler.getRequest() && scheduler.getDecoder() != null) {
            ...
            final ComStmtFetch stmtFetch = new ComStmtFetch();
            stmtFetch.decode(scheduler.getDecoder(), scheduler.getContext().getCapabilities());
            scheduler.setRequest(stmtFetch);
        }
        return null;
    }
}
