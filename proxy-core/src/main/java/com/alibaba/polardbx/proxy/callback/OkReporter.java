...
public class OkReporter implements Runnable {
    private static final Logger LOGGER = LoggerFactory.getLogger(OkReporter.class);

    private final FrontendConnection connection;
    private final FrontendContext context;

    public OkReporter(FrontendConnection connection, FrontendContext context) {
    ...
    @Override
    public void run() {
        context.sendOk(connection, false);
    }
}
