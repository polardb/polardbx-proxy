...
@Getter
@Setter
public class LocalInfileRequest implements MysqlPacket {
    /**
    ...
    public LocalInfileRequest() {
    }

    @Override
    public void decode(Decoder decoder, int ignored) {
        if (decoder.u8_s() != 0xFB) {
            throw new IllegalArgumentException("invalid packet type");
        }
        this.filename = decoder.str();
    }

    @Override
    public String toString() {
        return "LocalInfileRequest{" +
            "filename=" + (null == filename ? null : new String(filename)) +
            '}';
    }
}
