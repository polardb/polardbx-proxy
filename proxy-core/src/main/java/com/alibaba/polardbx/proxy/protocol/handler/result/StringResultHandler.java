...
public class StringResultHandler extends ResultHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(StringResultHandler.class);

    private byte[] statistics;

    public StringResultHandler(@NotNull AtomicReference<BackendContext> contextReference, Scheduler scheduler,
                               MysqlForwarder forwarder, ResultCallback stateCallback) {
        super(contextReference, scheduler, forwarder, stateCallback);
        setTag("StringResultHandler");
    }

    @Override
    public synchronized boolean handleAndTakePacket(Slice packet, Decoder decoder) {
        if (ResultState.Init == state) {
            statistics = decoder.str();
